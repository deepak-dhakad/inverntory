...
const LendenList = () => {
  ...
  const downloadPDF = () => {
    ...
    doc.autoTable({
      ...
      body: lendenRecords.map(({ name, description, date, transType, amount }) => [
        name,
        description,
    ...
    const totalCredit = lendenRecords.filter(r => r.transType === 'credit').reduce((sum, r) => sum + r.amount, 0);
    const totalDebit = lendenRecords.filter(r => r.transType === 'debit').reduce((sum, r) => sum + r.amount, 0);
  ...
  const handleDelete = async (id) => {
    ...
    if (!isConfirmed) return; // If the user cancels, stop the function here
      ...
      <div className="flex flex-col gap-4 sm:flex-row justify-center mb-4">
        ...
        <div className="flex flex-row gap-2 w-full">
          <div className="flex-1">
            <label className="text-gray-700 font-medium mb-1" htmlFor="start-date">Start Date</label>
            <input 
              type="date" 
              id="start-date" 
              value={startDate} 
          ...
          <div className="flex-1">
            <label className="text-gray-700 font-medium mb-1" htmlFor="end-date">End Date</label>
            <input 
              type="date" 
              id="end-date" 
              value={endDate} 
            ...
            {filteredRecords.map((record) => (
              <tr key={record._id} className="border-t">
                ...
                <td className="p-2 flex space-x-2">
                  <button onClick={() => openEditModal(record)} className="text-blue-500 hover:underline">Edit</button>
                  <button onClick={() => handleDelete(record._id)} className="text-red-500 hover:underline">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-4 sm:p-6 rounded-md shadow-md w-full sm:w-96">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <input type="text" name="name" placeholder="Name" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} required className="p-2 border border-gray-300 rounded-md w-full" />
              <input type="text" name="description" placeholder="Description" value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} className="p-2 border border-gray-300 rounded-md w-full" />
              <input type="date" name="date" value={editForm.date} onChange={(e) => setEditForm({ ...editForm, date: e.target.value })} required className="p-2 border border-gray-300 rounded-md w-full" />
              <select name="transType" value={editForm.transType} onChange={(e) => setEditForm({ ...editForm, transType: e.target.value })} required className="p-2 border border-gray-300 rounded-md w-full">
                <option value="credit">Credit</option>
                <option value="debit">Debit</option>
              </select>
              <input type="number" name="amount" placeholder="Amount" value={editForm.amount} onChange={(e) => setEditForm({ ...editForm, amount: e.target.value })} required className="p-2 border border-gray-300 rounded-md w-full" />
              <div className="flex justify-end gap-4">
                <button type="button" onClick={closeEditModal} className="p-2 bg-gray-500 text-white rounded-md">Cancel</button>
                <button type="submit" className="p-2 bg-blue-600 text-white rounded-md">Save Changes</button>
              </div>
            </form>
          </div>
        </div>
